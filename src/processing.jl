...
function processing(df::DataFrame)
    ...
    adjusted_vaccinated_first_last_week_deaths =
        vaccinated_first_last_week_deaths * FIRST_LAST_WEEK_ADJUSTMENT
    adjusted_vaccinated_other_last_week_deaths =
        vaccinated_other_last_week_deaths * OTHER_LAST_WEEK_ADJUSTMENT
    vaccinated_deaths =
    ...
    vaccinated_mortality =
        ...
        mean([vaccinated_individuals, (vaccinated_individuals - vaccinated_deaths)])
    ...
    unstandardized_dcci =
        [collect(zip(vaccinated_df[1:6, :DCCI], vaccinated_df[1:6, :days]))]
    ...
    unstandardized_vaccinated_mortality = # INFO: Correct
        deaths ./ mean([individuals, (individuals - deaths)])
    ...
    adjusted_unvaccinated_very_first_week_deaths =
        unvaccinated_very_first_week_deaths * VERY_FIRST_WEEK_ADJUSTMENT
    adjusted_unvaccinated_other_first_week_deaths =
        unvaccinated_other_first_week_deaths * OTHER_FIRST_WEEK_ADJUSTMENT
    adjusted_unvaccinated_first_last_week_deaths =
        unvaccinated_first_last_week_deaths * FIRST_LAST_WEEK_ADJUSTMENT
    adjusted_unvaccinated_other_last_week_deaths =
        unvaccinated_other_last_week_deaths * OTHER_LAST_WEEK_ADJUSTMENT
    unvaccinated_deaths =
        ...
        adjusted_unvaccinated_first_last_week_deaths +
        adjusted_unvaccinated_other_last_week_deaths
    dcci_mixed = [[(dcci, unvaccinated_accumulator[dcci+1]) for dcci = 0:5]]
    unvaccinated_mortality =
        [unvaccinated_deaths] ./ mean([
    ...
    total_after_standardization =
        sum(vaccinated_df[1:6, :individuals] .* especial_days_ratio)
    individuals_standardized_sum = sum(
        vaccinated_df[1:6, :individuals] ./ total_after_standardization .*
        total_before_standardization .* especial_days_ratio,
    )
    very_first_week_standardized_vaccinated_deaths = [
        sum(
            vaccinated_df[1:6, :very_first_week_deaths] ./ total_after_standardization .*
            total_before_standardization .* especial_days_ratio,
        ),
    ]
    other_first_week_standardized_vaccinated_deaths = [
        sum(
            vaccinated_df[1:6, :other_first_week_deaths] ./ total_after_standardization .* total_before_standardization .*
            especial_days_ratio,
        ),
    ]
    other_week_standardized_vaccinated_deaths = [
        sum(
            vaccinated_df[1:6, :other_week_deaths] ./ total_after_standardization .*
            total_before_standardization .* especial_days_ratio,
        ),
    ]
    first_last_week_standardized_vaccinated_deaths = [
        sum(
            vaccinated_df[1:6, :first_last_week_deaths] ./ total_after_standardization .*
            total_before_standardization .* especial_days_ratio,
        ),
    ]
    other_last_week_standardized_vaccinated_deaths = [
        sum(
            vaccinated_df[1:6, :other_last_week_deaths] ./ total_after_standardization .*
            total_before_standardization .* especial_days_ratio,
        ),
    ]
    standardized_vaccinated_deaths = [
        sum(
            vaccinated_df[1:6, :deaths] ./ total_after_standardization .*
            total_before_standardization .* especial_days_ratio,
        ),
    ]
    standardized_vaccinated_mortality =
        standardized_vaccinated_deaths ./
        mean([individuals, (individuals - standardized_vaccinated_deaths)])
...
# Processing
processed = Dict(ThreadsX.map(((k, df),) -> k => processing(df), collect(exact_selection)))
